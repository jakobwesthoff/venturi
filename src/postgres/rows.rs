...
pub(crate) fn job_from_row(row: &Row) -> Result<JobRecord, Error> {
    ...
    Ok(JobRecord {
        ...
        // The DB column is signed `integer`; run counts only ever start at 0 and
        // increment, so the value is non-negative and `as u32` is exact.
        run_count: row.get::<_, i32>(10) as u32,
...
pub(crate) fn journal_from_row(row: &Row) -> Result<JournalRecord, Error> {
    ...
    Ok(JournalRecord {
        ...
        job_id,
        kind: row.get(2),
        // Signed `integer` column; a run number is always >= 1, so `as u32` is exact.
        run_no: row.get::<_, i32>(3) as u32,
